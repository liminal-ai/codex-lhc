...
fail=0
source scripts/lhc-tripwire-common.sh
...
check_vendor_clean() {
  ...
  local vendor_status
  if ! vendor_status=$(git -C "$vendor" status --porcelain 2>"$lhc_log_dir/lhc-vendor-git.err"); then
    echo "TRIPWIRE vendor[$phase]: git status failed in $vendor"
    cat "$lhc_log_dir/lhc-vendor-git.err" 2>/dev/null | head -10
    return 1
  fi
  ...
  local pin
  if ! pin=$(git -C "$vendor" log -1 --format=%h 2>"$lhc_log_dir/lhc-vendor-git.err"); then
...
if cargo check -q -p codex-core -p codex-app-server -p codex-extension-api \
    --manifest-path codex-rs/Cargo.toml >"$lhc_log_dir/lhc-hook-check.log" 2>&1; then
  ...
  grep -E "^error" -A5 "$lhc_log_dir/lhc-hook-check.log" | head -40
  fail=1
fi
...
if cargo build -q -p codex-cli --manifest-path codex-rs/Cargo.toml \
    >"$lhc_log_dir/lhc-hook-bare-build.log" 2>&1 \
    && scripts/check-lhc-default-capture.py \
        --binary codex-rs/target/debug/codex \
        >"$lhc_log_dir/lhc-hook-bare-capture.log" 2>&1; then
  cat "$lhc_log_dir/lhc-hook-bare-capture.log"
else
  echo "TRIPWIRE bare-exec: default LHC capture did not persist a complete turn:"
  cat "$lhc_log_dir/lhc-hook-bare-build.log" "$lhc_log_dir/lhc-hook-bare-capture.log" 2>/dev/null | tail -60
  fail=1
fi
...
if [ -x codex-rs/target/debug/codex ] \
    && scripts/check-lhc-midturn-parts.py \
        --binary codex-rs/target/debug/codex \
        >"$lhc_log_dir/lhc-hook-midturn-parts.log" 2>&1; then
  cat "$lhc_log_dir/lhc-hook-midturn-parts.log"
else
  echo "TRIPWIRE midturn-parts: bare exec did not cross a mid-turn parts compact:"
  tail -60 "$lhc_log_dir/lhc-hook-midturn-parts.log" 2>/dev/null
  fail=1
fi
...
if just test --retries 0 -p codex-lhc-host --lib \
  ...
  grep -E "^error|FAIL|panicked" -A3 "$lhc_log_dir/lhc-hook-lib.log" | head -30
  fail=1
fi
...
if env -u UPDATE_LHC_GOLDENS just test --retries 0 -p codex-lhc-host --features test-util \
  ...
  grep -E "^error|FAIL|panicked" -A5 "$lhc_log_dir/lhc-hook-cert.log" | head -40
  fail=1
fi
...
if just test --retries 0 -p codex-core --lib lhc_capture_e2e \
  ...
  grep -E "^error|FAIL|panicked" -A5 "$lhc_log_dir/lhc-hook-e2e.log" | head -40
  fail=1
fi
...
if just test --retries 0 -p codex-core --lib config_schema_matches_fixture \
  ...
  grep -E "^error|FAIL|panicked" -A5 "$lhc_log_dir/lhc-hook-schema.log" | head -30
  fail=1
fi
...
if just test --retries 0 -p codex-lhc-host --lib compact_bridge \
  ...
  grep -E "^error|FAIL|panicked" -A5 "$lhc_log_dir/lhc-hook-bridge.log" | head -40
  fail=1
fi
...
if RUST_MIN_STACK=8388608 just test --retries 0 -p codex-core --lib 'compact_lhc::tests::' \
  ...
  grep -E "^error|FAIL|panicked" -A5 "$lhc_log_dir/lhc-hook-arm.log" | head -40
  fail=1
fi
...
if just test --retries 0 -p codex-core --lib 'compact_lhc::mid_turn_tests::' \
  ...
  grep -E "^error|FAIL|panicked" -A5 "$lhc_log_dir/lhc-hook-midturn.log" | head -40
  fail=1
fi
...
if cargo clippy -q -p codex-lhc-host --lib --no-deps --manifest-path codex-rs/Cargo.toml \
    -- -D unused -D dead_code >"$lhc_log_dir/lhc-hook-clippy.log" 2>&1; then
  echo "ok clippy: codex-lhc-host --no-deps (-D unused -D dead_code)"
else
  echo "TRIPWIRE clippy: codex-lhc-host failed:"
  grep -E "^error|warning:" "$lhc_log_dir/lhc-hook-clippy.log" | head -40
  fail=1
fi
...
if [ ! -f "$base_file" ]; then
  ...
  # The drill itself.
  scratch=$(mktemp -d)
  if ! git worktree add --detach "$scratch" "$patch_base" >"$lhc_log_dir/lhc-patch-wt.log" 2>&1; then
    echo "TRIPWIRE patch-repro: git worktree add at $patch_base failed"
    head -10 "$lhc_log_dir/lhc-patch-wt.log"
    repro_fail=1
  else
    applied=1
    for p in $series; do
      if ! (cd "$scratch" && git apply "$main_root/$p" >"$lhc_log_dir/lhc-patch-apply.log" 2>&1); then
        echo "TRIPWIRE patch-repro: $p failed to apply at base $patch_base"
        head -20 "$lhc_log_dir/lhc-patch-apply.log"
        applied=0
        repro_fail=1
        break
      fi
...
if just test --retries 0 -p codex-core --lib slice_d_ \
  ...
  grep -E "^error|FAIL|panicked" -A5 "$lhc_log_dir/lhc-hook-drill.log" | head -40
  fail=1
fi

# SDK ancestry: one refresh; unavailable evidence remains explicitly unverified.
pin_status=0
lhc_check_pin "$vendor" || pin_status=$?
...
lhc_report_result "$fail" "$pin_status"
exit $?
