    ...
    try:
        ...
        expect_success(
            sandbox.process.exec(
                f"powershell -NoProfile -Command \"New-Item -Force -ItemType Directory '{remote}' | Out-Null\""
            ),
            "create candidate directory",
        )
        sandbox.fs.upload_files(
            [
                FileUpload(source=str(path), destination=rf"{remote}\{path.name}")
                for path in required
            ]
            + [
                FileUpload(
                    source=str(capture_probe),
                    destination=rf"{remote}\{capture_probe.name}",
                )
            ]
        )
        install = (
        ...
        expect_success(
            sandbox.process.exec(
                r"cmd /c C:\codex-lhc-prefix\bin\codex-lhc-smoke.cmd --version"
            ),
            "codex --version",
        )
        expect_success(
            sandbox.process.exec(
                r"cmd /c C:\codex-lhc-prefix\bin\codex-lhc-smoke.cmd --help > C:\codex-help.txt"
            ),
            "codex --help",
        )
        expect_success(
            sandbox.process.exec(
                rf"cmd /c C:\codex-lhc-packages\versions\{version}\bin\codex-code-mode-host.exe --help > C:\host-help.txt"
            ),
            "code-mode host --help",
        )
        expect_success(
        ...
        expect_success(
            sandbox.process.exec(
                r'powershell -NoProfile -Command "New-Item -Force -ItemType Directory C:\codex-lhc-data | Out-Null; Set-Content C:\codex-lhc-data\smoke-marker preserve"'
            ),
            "create data marker",
        )
        uninstall = (
