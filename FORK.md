...
- `patches/lhc/` — re-appliable patch per core touchpoint, regenerated by
  `patches/lhc/regenerate.sh` (see its README).
...
4. **Advance the patch base.** `patches/lhc/BASE` names the upstream commit the
   ...
   fails. Write the merged upstream tip into `BASE` and regenerate the whole
   series against it (`sh patches/lhc/regenerate.sh`; see
   `patches/lhc/README.md`). This step is **part of the sync**, not cleanup
   after it.
...
Current upstream base: `patches/lhc/BASE` (`rust-v0.157.1` at this revision).

2026-09-26 `rust-v0.157.1` merge: merged peeled `36650394c5` onto the 0.157.0-lhc.1 tree (`8b6a686c2c`) as `ea33ea5c77`. 1 file conflicted: `Cargo.toml` (workspace version). Upstream delta is four Windows-only fixes (code-mode host and stdio MCP console windows; daemon launch job membership and stdio inheritance), all `cfg(windows)`; no hooked or patch-owned file touched; `Cargo.lock` regenerated (155 workspace version lines). `BASE` to `36650394c5`. LHC core pin `47e360a3` and thread schema 13 unchanged; no store migration.

2026-09-25 `rust-v0.157.0` merge: merged peeled `00c972ed5d` onto the 0.156.1 LHC tree (`cdee7779f0`). 21 files conflicted: `rust-release-prepare.yml` (deleted upstream, resolved as deleted), `Cargo.toml` (workspace version), `session/mod.rs` (upstream builds the native `CompactedItem` inside the state lock with `resume_metadata`; fork's `guardian_covered_suffix_items: Some(0)` and the I2 durable-record line kept inside it), `rollout_reconstruction.rs` (upstream's `select_input_compaction` rewrite; the Guardian overlap block re-pointed from `base_compaction` to `history_checkpoint`), `turn.rs` (upstream moved `responses_metadata`; `LhcStepIndex::begin_cycle` stays before the request send), `history/src/lib.rs` (both module headers; `CompactedItem` carries both new fields), `state/src/migrations.rs` (upstream `sqlx_macros::migrate!`; fork repair routine kept), `models.json` (upstream catalog incl. GPT-6 Sol/Luna at upstream windows; fork windows re-applied to gpt-5.6-sol/terra/luna), two Guardian test files (LHC registration kept, upstream test edits taken; the native tool-call assertion dropped again per 1af4c39006), two TUI test files, nine snapshots (regenerated). Fork positions taken with the merge (Lee, 2026-09-25): `daemon_auto_start` default OFF in the fork (upstream 75ec81c862 turns it on); `guardianv2.thread_context` left at upstream's new default ON (ThreadOwned) with the overlap carry as built; the LHC fold now writes `resume_metadata` (`Session::compaction_resume_metadata`) so 0.157.0 cold replay selects the fold as its input checkpoint. Thread-history migration: upstream added its own `0007` (item lifecycle timestamps), so the fork's rollout-generation migration moved to `0008`; `repair_legacy_rollout_generation_migration_version` now relabels any `_sqlx_migrations` row carrying the generation checksum (version 5 from pre-0.155.1 homes, version 7 from 0.155.1/0.156.1 homes) to 8 before migrating. One-way for rollback: a home opened by 0.157.0 holds upstream 7, which 0.156.1 does not recognise; rollback restores the backed-up thread-history DB. `BASE` to `00c972ed5d`. SDK pin `47e360a3` unchanged. `lhc-release/VERSION` unchanged until the release cut.
...
sync regenerates the entire series against the new `patches/lhc/BASE`. Both
run `sh patches/lhc/regenerate.sh`; see
