...
```bash
sh patches/lhc/regenerate.sh
```

[`regenerate.sh`](regenerate.sh) reads the ONE base recorded in
`patches/lhc/BASE` and runs one `git diff "$BASE" -- <files>` per group in
the table below; the file lists live in the script and are the ownership
record. Regenerating against whatever HEAD happens to be is what broke this
series through Chunk 2 (FORK.md §History-reset R1/R2). The script reads the
working tree and does not touch your index, so `git add` any newly created
fork-owned file first (as part of normal staging) and add it to the right
group in the script; `git diff <base>` does not see untracked files, and
tripwire layer 4's coverage check is the backstop for one you forgot.
...
## Series (regenerated 2026-09-06 against `BASE` = `3d2ee51ca2`, stable `rust-v0.153.4`)
...
One base for all eight; each fork-owned file in exactly one patch.

| Patch | Files |
|-------|-------|
| 0001-workspace-member | `codex-rs/Cargo.toml`, `cli/tests/version.rs` (upstream workspace-version regression; `cli/Cargo.toml` itself is restored to upstream), `app-server-daemon/{README.md,src/lib.rs,src/managed_install.rs,src/managed_install_tests.rs,src/update_loop.rs,src/update_loop_tests.rs}` (managed version coherence + isolated-home fork-byte seed), `windows-sandbox-rs/src/{desktop_tests.rs,lib.rs,winutil.rs}` (Windows voice: token-resolved account name) |
| 0002-raw-item-contributor | `ext/extension-api/{contributors.rs, contributors/raw_item.rs, contributors/turn_lifecycle.rs, lib.rs, registry.rs}`, `ext/goal/tests/goal_extension_backend.rs` (turn timing fields on lifecycle inputs), `ext/mcp/src/cloud_plugin_tests.rs` (`started_at` on the cloud-plugin fixture) |
...
| 0006-app-server-install | `app-server/src/extensions.rs`, `app-server/src/lib.rs`, `app-server/src/in_process.rs` (install + process-wide claim hand-back after thread shutdown) |
| 0007-lhc-compact-arm | `core/Cargo.toml`, `.config/nextest.toml`, `core/src/compact_lhc/{preparation,workers,installation}.rs`, `core/src/{compact_lhc.rs,compact_lhc_worker_error.rs,compact_lhc_tests.rs,compact_lhc_strict_routing_tests.rs,compact_lhc_slice_d_tests.rs,compact_lhc_mid_turn_tests.rs,compact_lhc_canary_tests.rs,compact_lhc_readiness_tests.rs,lc_adaptive_service_tier.rs,lhc_inference_bridge.rs,lib.rs,thread_manager.rs}`, `core/src/config/{mod.rs,config_tests.rs}`, `core/src/context_manager/history.rs`, `core/src/guardian/review_session.rs`, `ext/guardian-v2/src/async_scorer/{approval.rs,config.rs,observation.rs,parent_compaction.rs,parent_compaction_tests.rs}`, `ext/guardian-v2/src/sync_reviewer/{lifecycle_tests.rs,reviewer_config.rs}`, `core/tests/suite/{compact.rs,compact_lhc_mid_turn_loops.rs,compact_lhc_image_tests.rs,compact_lhc_context_management.rs,compact_remote.rs,compact_resume_fork.rs,client.rs,current_time_reminder.rs,hooks.rs,lhc_preturn_readiness.rs,pending_input.rs,remote_env.rs,responses_lite.rs,retry_after.rs,rollout_budget.rs,token_budget.rs,tools.rs,window_headers.rs,model_switching.rs,mod.rs}`, `core/src/session/{turn.rs,session.rs,tests.rs,input_queue.rs,lhc_band_shape_eval_tests.rs}`, `core/src/state/{auto_compact_window.rs,session.rs,service.rs}`, `core/src/tasks/{compact.rs,lifecycle.rs,mod.rs}`, `exec/src/{lib.rs,lib_tests.rs,event_processor_with_human_output_tests.rs}`, `exec/tests/suite/{apply_patch.rs,auth_env.rs,resume.rs}`, `config/src/config_toml.rs`, `protocol/src/{config_types.rs,openai_models.rs}`, `models-manager/{models.json,src/manager.rs,src/manager_tests.rs}`, `history/src/{lib.rs,ordinal.rs}`, `rollout/src/{lib.rs,ordinal.rs,recorder.rs,recorder_tests.rs}`, `state/{src/migrations.rs,src/migrations_tests.rs,src/sqlite.rs,thread_history_migrations/0007_rollout_generation_id.sql}`, `thread-store/Cargo.toml`, `thread-store/src/live_thread.rs`, `thread-store/src/local/{mod.rs,live_writer.rs,rollout_migration.rs,rollout_migration_tests.rs,thread_history.rs,thread_history_generation.rs,thread_history_materialization.rs,thread_history_materialization_tests.rs}`, `app-server/src/request_processors/thread_processor.rs` (slice E reconcile), `app-server/tests/suite/v2/history_notes_extension.rs` (notes/reset compatibility). LIM-63B/LIM-67 MidTurn compact-continuation, LC Adaptive Service Tier, rewrite ordinal continuity, generation-identity projection recovery, Guardian v2 LHC omit, and LIM-134 PreTurn readiness / prompt preservation / exec empty-result truth live here; host adapter code is under `codex-rs/lhc/` (not patched). |
| 0008-release-identity-update-wiring | `BUILD.bazel` (repo root: exports `lhc-release/VERSION`), `install-context/{BUILD.bazel,src/lhc_release.rs,src/lhc_release_tests.rs,src/lib.rs}` (embedded `LHC_RELEASE_VERSION`, `parse_lhc_release`, fork URL constants), `cli/src/main.rs` (`--lhc-version`, `update` through the fork installer, manual instructions), `cli/src/doctor/{updates.rs,output.rs}` (fork update row and note), `tui/src/{update_action.rs,update_action_tests.rs,update_prompt.rs,update_versions.rs,updates.rs,updates_cache.rs,updates_cache_tests.rs,app/exit_summary.rs,history_cell/mod.rs,history_cell/notices.rs,history_cell/tests.rs}` and the seven update prompt/notice snapshots under `tui/src/snapshots/` and `tui/src/history_cell/snapshots/` (managed-install detection, installer-run update action, fork release discovery, `lhc-version.json` cache, advice). Maintenance slices 01-06, 2026-09-06. |
