...
## Series (regenerated 2026-08-27 against `BASE` = `9085439396`, stable `rust-v0.150.1`)
...
| Patch | Files |
|-------|-------|
| 0001-workspace-member | `codex-rs/Cargo.toml`, `code-mode-runtime/Cargo.toml` (local V8 artifact workaround), `cli/tests/version.rs` (upstream workspace-version regression; `cli/Cargo.toml` itself is restored to upstream), `app-server-daemon/{README.md,src/lib.rs,src/managed_install.rs,src/managed_install_tests.rs,src/update_loop.rs,src/update_loop_tests.rs}` (managed version coherence + isolated-home fork-byte seed) |
...
| 0007-lhc-compact-arm | `core/Cargo.toml`, `core/src/{compact_lhc.rs,compact_lhc_tests.rs,compact_lhc_strict_routing_tests.rs,compact_lhc_slice_d_tests.rs,compact_lhc_mid_turn_tests.rs,compact_lhc_canary_tests.rs,lc_adaptive_service_tier.rs,lhc_inference_bridge.rs,lib.rs,thread_manager.rs}`, `core/src/config/{mod.rs,config_tests.rs}`, `core/tests/suite/{compact.rs,compact_lhc_mid_turn_loops.rs,compact_remote.rs,compact_remote_parity.rs,compact_resume_fork.rs,client.rs,current_time_reminder.rs,hooks.rs,pending_input.rs,remote_env.rs,responses_lite.rs,retry_after.rs,rollout_budget.rs,token_budget.rs,tools.rs,window_headers.rs,model_switching.rs,mod.rs}`, `core/src/session/{turn.rs,session.rs,tests.rs,input_queue.rs,lhc_band_shape_eval_tests.rs}`, `core/src/state/{auto_compact_window.rs,session.rs,service.rs}`, `core/src/tasks/{compact.rs,lifecycle.rs,mod.rs}`, `config/src/config_toml.rs`, `protocol/src/{config_types.rs,openai_models.rs}`, `models-manager/{models.json,src/manager.rs,src/manager_tests.rs}`, `history/src/{lib.rs,ordinal.rs}`, `rollout/src/{lib.rs,ordinal.rs,recorder.rs,recorder_tests.rs}`, `state/{src/migrations.rs,src/migrations_tests.rs,src/sqlite.rs,thread_history_migrations/0007_rollout_generation_id.sql}`, `thread-store/Cargo.toml`, `thread-store/src/live_thread.rs`, `thread-store/src/local/{mod.rs,live_writer.rs,rollout_migration.rs,rollout_migration_tests.rs,thread_history.rs,thread_history_generation.rs,thread_history_materialization.rs,thread_history_materialization_tests.rs}`, `app-server/src/request_processors/thread_processor.rs` (slice E reconcile). LIM-63B/LIM-67 MidTurn compact-continuation, LC Adaptive Service Tier, rewrite ordinal continuity, and generation-identity projection recovery live here; host adapter code is under `codex-rs/lhc/` (not patched). |
