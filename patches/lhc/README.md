...
| Patch | Files |
|-------|-------|
| 0001-workspace-member | `codex-rs/Cargo.toml`, `code-mode-runtime/Cargo.toml` (local V8 artifact workaround), `cli/tests/version.rs` (upstream workspace-version regression; `cli/Cargo.toml` itself is restored to upstream), `app-server-daemon/src/{lib.rs,managed_install_tests.rs}` (managed version coherence) |
