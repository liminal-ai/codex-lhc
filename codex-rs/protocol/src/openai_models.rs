...
impl ModelInfo {
    ...
    pub fn auto_compact_token_limit(&self) -> Option<i64> {
        ...
        if let Some(configured_limit) = self.auto_compact_token_limit {
            return Some(
                context_window.map_or(configured_limit, |window| configured_limit.min(window)),
            );
        }
        context_window.map(|window| (window * 9) / 10)
    }
...
mod tests {
    ...
    #[test]
    fn explicit_auto_compact_limit_can_use_reserved_headroom_above_ninety_percent() {
        let model = ModelInfo {
            context_window: Some(370_000),
    ...
    #[test]
    fn explicit_auto_compact_limit_never_exceeds_working_window() {
        let model = ModelInfo {
            context_window: Some(370_000),
            max_context_window: Some(1_050_000),
            auto_compact_token_limit: Some(500_000),
            ..test_model(/*spec*/ None)
        };

        assert_eq!(model.auto_compact_token_limit(), Some(370_000));
    }

    // codex-lhc: an explicit `auto_compact_token_limit` is honored up to the
    // resolved window (fork policy above), so the configured 250_000 stands
    // instead of upstream's 90% clamp (244_800).
    #[test]
