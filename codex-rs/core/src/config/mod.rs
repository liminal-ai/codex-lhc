...
impl Config {
    ...
    pub(crate) async fn load_config_with_layer_stack(
        ...
        if lc_adaptive_service_tier.threshold <= 0 {
            return Err(std::io::Error::new(
                std::io::ErrorKind::InvalidInput,
                "lc_adaptive_service_tier.threshold must be positive",
            ));
        }
        for (field, value) in [
            ("below", &mut lc_adaptive_service_tier.below),
            ("at_or_above", &mut lc_adaptive_service_tier.at_or_above),
        ] {
            if value == SERVICE_TIER_DEFAULT_REQUEST_VALUE {
                continue;
            }
            let Some(tier) = ServiceTier::from_request_value(value) else {
                return Err(std::io::Error::new(
                    std::io::ErrorKind::InvalidInput,
                    format!(
                        "lc_adaptive_service_tier.{field} must be fast, priority, flex, or default"
                    ),
                ));
            };
            *value = tier.request_value().to_string();
        }
