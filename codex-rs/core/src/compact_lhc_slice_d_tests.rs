...
// ── M1: named unpaired FunctionCallOutput through the real compact arm ─────

fn named_unpaired_output(body: codex_protocol::models::FunctionCallOutputBody) -> ResponseItem {
    ResponseItem::FunctionCallOutput {
        id: None,
        call_id: None,
...
fn function_call_outputs(items: &[ResponseItem]) -> Vec<&ResponseItem> {
    items
        .iter()
...
/// Provider-visible `FunctionCallOutput` shape: host-stamped chat passthrough
/// is stripped before a non-OpenAI send and is not part of the contract this
/// test proves. Assigned item identity is kept.
fn provider_visible_function_call_output(item: &ResponseItem) -> ResponseItem {
    let mut item = item.clone();
    item.clear_internal_chat_message_metadata_passthrough();
    item
}

/// Production path: a named unpaired `FunctionCallOutput` (`call_id: None`,
/// nonempty name and namespace) recorded on a live session survives capture,
/// the LHC compact generation rewrite, reopen/materialize, and the next
/// provider request with its original absent call id, exact name / namespace /
/// payload, and the host-assigned item identity — never a paired tool result
/// with a synthetic call id.
#[tokio::test]
async fn slice_d_named_unpaired_function_output_survives_generation_rewrite() {
    let _swap = hold_swap_clean();
    let dir = tempdir().unwrap();
    let root = dir.path().to_path_buf();
    let (mut session, tc) = make_session_and_context().await;
    ...
    let precompact = session.clone_history().await.into_raw_items();
    let precompact_outputs = function_call_outputs(&precompact);
    assert_eq!(precompact_outputs.len(), 1, "{precompact_outputs:?}");
    let ResponseItem::FunctionCallOutput {
        id: Some(precompact_id),
        call_id: None,
        name: Some(name),
        namespace: Some(namespace),
        output,
        ..
    } = precompact_outputs[0]
    else {
        panic!(
            "host-recorded unpaired output must keep call_id: None with assigned item identity: {:?}",
            precompact_outputs[0]
        );
    };
    assert!(
        precompact_id.is_prefixed(),
        "host-assigned item identity is provider-visible: {precompact_id}"
    );
    assert_eq!(name, "notifications");
    assert_eq!(namespace, "slack");
    assert_eq!(output.success, Some(true));
    assert!(matches!(
        output.body,
        codex_protocol::models::FunctionCallOutputBody::Text(ref text)
            if text == "parent notification"
    ));
    let expected = provider_visible_function_call_output(precompact_outputs[0]);

    let sess = Arc::new(session);
    let attempt = run_arm_deterministic(&sess, &tc, /*manual*/ true).await;
    let LhcCompactAttempt::Installed { body, .. } = attempt else {
        panic!("expected an installed compact: {attempt:?}");
    };

    let installed = function_call_outputs(&body)
        .into_iter()
        .map(provider_visible_function_call_output)
        .collect::<Vec<_>>();
    assert_eq!(
        installed,
        vec![expected.clone()],
        "installed body keeps the host-recorded unpaired output, including assigned item identity"
    );

    // Reopen: rebuild the next provider request from the rewritten generation.
    // `FunctionCallOutputPayload.success` is in-memory metadata and is not on
    // the function_call_output wire, so disk reopen deserializes it as None.
    // Compare the provider-visible JSON (includes assigned id; omits success).
    let items = parse_rollout_items(&rollout_path).expect("parse rewritten rollout");
    let rebuilt = reconstruct_model_history(&items);
    let reopened = function_call_outputs(&rebuilt)
        .into_iter()
        .map(provider_visible_function_call_output)
        .collect::<Vec<_>>();
    assert_eq!(
        serde_json::to_value(&reopened).expect("serialize reopened outputs"),
        serde_json::to_value(&installed).expect("serialize installed outputs"),
        "reopen provider-visible JSON matches the installed host-recorded output, including assigned item identity"
    );
    assert_eq!(
        serde_json::to_value(&reopened).expect("serialize reopened outputs"),
        serde_json::to_value(std::slice::from_ref(&expected)).expect("serialize expected outputs"),
        "reopen provider-visible JSON matches the precompact host-recorded output"
    );
    let rebuilt_json = serde_json::to_string(&rebuilt).expect("serialize rebuilt body");
    assert!(
        !rebuilt_json.contains("synthetic:"),
        "no synthetic call id reaches the provider body: {rebuilt_json}"
    );
}

/// Production path: a variant the closed LHC `tool_result` payload cannot carry
/// exactly is refused before anything is installed — the session keeps the body
/// it already holds and the rollout is not rewritten.
#[tokio::test]
async fn slice_d_unrepresentable_unpaired_output_refuses_before_install() {
    let _swap = hold_swap_clean();
    let dir = tempdir().unwrap();
    let root = dir.path().to_path_buf();
    let (mut session, tc) = make_session_and_context().await;
    ...
    let before = std::fs::read(&rollout_path).expect("read rollout before");
    let history_before = session.clone_history().await.into_raw_items();

    let sess = Arc::new(session);
    let attempt = run_arm_deterministic(&sess, &tc, /*manual*/ true).await;
    let LhcCompactAttempt::ContinuedWithoutCompact { reason } = attempt else {
        panic!("expected a visible refusal, got: {attempt:?}");
    };
    assert!(
        reason.contains("cannot represent captured items exactly")
            && reason.contains("notifications"),
        "refusal must name the cause: {reason}"
    );
    assert_eq!(
        std::fs::read(&rollout_path).expect("read rollout after"),
        before,
        "no generation rewrite on refusal"
    );
    assert!(
        ...
        "the session keeps the body it already held"
    );
}

/// Q1: a malformed authority line means the complete eligible realtime set
/// cannot be proven. Compact refuses before materialization or rewrite.
#[tokio::test]
async fn slice_d_unprovable_realtime_rows_refuse_before_install() {
    let _swap = hold_swap_clean();
    let dir = tempdir().unwrap();
    let root = dir.path().to_path_buf();
    let (mut session, tc) = make_session_and_context().await;
    ...
    let mut before = std::fs::read(&rollout_path).expect("read rollout before");
    before.extend_from_slice(b"{not-json\n");
    std::fs::write(&rollout_path, &before).expect("append malformed authority line");
    let history_before = session.clone_history().await.into_raw_items();
    ...
    assert!(
        reason.contains("prior rollout realtime rows unreadable"),
        "refusal must name the unprovable realtime set: {reason}"
    );
    assert_eq!(
        std::fs::read(&rollout_path).expect("read rollout after"),
        before,
        "no generation rewrite on unprovable realtime authority"
    );
    assert!(
        response_items_structurally_equal(
            &sess.clone_history().await.into_raw_items(),
