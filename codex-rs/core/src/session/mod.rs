...
impl Session {
    ...
    /// Resume metadata for an LHC fold's `Compacted` record, built from the
    /// same session state the native compaction write reads.
    pub(crate) async fn compaction_resume_metadata(&self) -> CompactionResumeMetadata {
        let state = self.state.lock().await;
    ...
    async fn record_prepared_conversation_items(
        ...
        self.send_raw_response_items(turn_context, &response_items, provenance)
            .await;
    }
    ...
    pub(crate) async fn replace_compacted_history(
        ...
        let compacted_item = {
            ...
            CompactedItem {
                ...
                // Native compact writes Some(0); LHC folds write None (see rollout_reconstruction).
